...
## Procesamiento de archivos (ahorro de tokens)

### PDFs
**Texto corrido** (contratos, notas, manuales): extraer primero con pdftotext archivo.pdf - o markitdown archivo.pdf en vez de leer el PDF directamente.
**Tablas de layout fijo** (ej. "Diseño de Registro" de ARCA/AFIP, specs de campos con posiciones Desde/Hasta): usar pdftotext -layout archivo.pdf - y verificar que Desde/Hasta/Cant. sean consistentes entre filas (Desde = Hasta anterior + 1; Cant. = Hasta − Desde + 1). Si la extracción se ve desalineada o inconsistente, leer el PDF directamente como imagen en vez de confiar en el texto extraído — en estos documentos un error de posición de campo es costoso.
Si el PDF es escaneado (sin capa de texto), pdftotext/markitdown van a devolver texto vacío o basura — en ese caso requiere OCR (tesseract) o lectura como imagen.

### ZIPs
Listar contenido antes de extraer: unzip -l archivo.zip (o zipinfo -1 archivo.zip).
Extraer a una carpeta de scratch, no al proyecto.
Si solo se necesitan algunos archivos, extraer solo esos: unzip archivo.zip ruta/al/archivo -d destino.
Para buscar algo puntual sin extraer todo: unzip -p archivo.zip archivo | grep patron (o zgrep para .gz).
Una vez extraídos, aplicar las reglas de PDF/otros tipos de archivo de arriba a cada archivo individual.

### Regla general
Priorizar ahorro de tokens (extracción de texto en vez de lectura directa) salvo en documentos donde un error de interpretación sea costoso (specs fiscales, layouts de registro) — ahí priorizar exactitud sobre ahorro, verificando la extracción o leyendo como imagen si hace falta.
